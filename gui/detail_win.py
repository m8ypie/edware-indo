...
TRACKER_0_STATUS = "Di permukaan yang dapat memantulkan"
TRACKER_1_STATUS = "Di permukaan yang tidak dapat memantulkan"

#                           title,      (mod,bit),       if_variant
EVENT_NAME = ["Line Tracker","Data from TV remote","Mendeteksi hambatan","Data from another Edison","Mendeteksi tepukan","Music","Drive","Countdown timer","Keypad"]
EVENT_DICT = {'Keypad': (
              ...
              'Detect clap' : (
                  ('tepukan terdeteksi', (None, 2), 'clap'),),
              'Data from another Edison': (
                  ('IR Character', (None, 0), 'irrx'),),
              'Detect obstacle' : (
                  ('hambatan di depan', (None,4), 'obstacle'),
                  ('hambatan di kiri', (None,5), 'obstacle'),
                  ('hambatan di kanan', (None,3), 'obstacle'),
                  ('hambatan terdeteksi', (None,6), 'obstacle'),),
              'Data from TV remote': (
...
MOTOR_P_RT = "Maju kanan"
MOTOR_P_LT = "Maju kiri"
MOTOR_P_BL = "Mundur kiri"
MOTOR_P_BR = "Mundur kanan"
MOTOR_P_SR = "Berputar ke kiri"
MOTOR_P_SL = "Berputar ke kanan"
MOTOR_P_RT_90 = "Belok ke kanan 90 derajat"
MOTOR_P_LT_90 = "Belok ke kiri 90 derajat"
...
##            self.vbox.Add(hbox, flag=wx.EXPAND)
    ...
    def make_headings(self, grid, loc):
        grid.Add(wx.StaticText(self, -1, "Konstan"), loc, flag=wx.ALIGN_CENTRE)
        loc = (loc[0], loc[1]+1)
        grid.Add(wx.StaticText(self, -1, "Variabel"), loc, flag=wx.ALIGN_CENTRE)
...
# ---------------------------- digital, ctrlled, linetracker, onstacle detect brics ----------------------------------------
    ...
    def digital_details(self, bric_id, data):
        ...
        if (self.name == 'LED'):
            ...
            levels = ['Menyala', 'Mati']
            prompt = "Transmiter IR untuk Deteksi hambatan:"
        else:
            mod_type = 'Line Tracker'
            levels = ['Menyala', 'Mati']
            prompt = "Layar Line tracker:"
...
# ---------------------------- CPU bric ----------------------------------------
    ...
    def cpu_details(self, bric_id, data):
        ...
        if (self.prop_title):
            ...
            self.title.SetLabel("%s - !25properties:" % (self.name))
...
# -------------- Read Motor distance details  --------------------
    ...
    def readdist_details(self, bric_id, data):
        ...
        self.name = win_data.program().get_bric_name(bric_id)
        self.prop_title = bric_data.get_bric_prop_title(self.name)
        self.module_aliases = [("Motor kiri", "Left drive"), ("Motor kanan", "Right drive")]
...
# ---------------------------- Motor Pairs, Motors brics ----------------------------------------
    ...
    def motor_details(self, bric_id, data):
        ...
        self.make_headings(grid, (grid_line,1))
        self.add_with_prompt(grid, (grid_line+1,0), "Arah:", dirs)
        self.add_with_prompt(grid, (grid_line+2,0), "Kecepatan:", speed)
...
# ---------------------------- Event/Wait/If/Loop Brics ----------------------------------------
    ...
    def get_event_choices(self, name):
        ...
        if (name == "Mendeteksi hambatan"):
            name = "Detect obstacle"
        if (name == "Mendeteksi tepukan"):
            name = "Detect clap"
    ...
    def event_details(self, bric_id, data):
        ...
        grid.Add(wx.StaticText(self, -1, "Peristiwa terjadi:"), (0,1), flag=wx.ALIGN_LEFT)
    ...
    def loop_details(self, bric_id, data):
        ...
        rbs = self.make_radio_buttons(["Ujian terlewati", "Peristiwa terjadi", "Loop selamanya atau mengulangi selamanya"])
        ...
        grid.Add(wx.StaticText(self, -1, "Loop sampai atau mengulangi sampai:"), (0,1), flag=wx.ALIGN_LEFT)
